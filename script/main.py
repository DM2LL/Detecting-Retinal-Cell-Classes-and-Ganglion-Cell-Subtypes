...
parser.add_argument('--source_name', type=str, default='Tran_source')
parser.add_argument('--target_name', type=str, default='Tran_target')
parser.add_argument('--result_path', type=str, default='Results/')
parser.add_argument('--dataset_path', type=str, default='dataset/Tran/')
parser.add_argument('--num_iterations', type=int, default=25000, help="num_iterations")
...
def preprocess(args):
    ...
    datacounts = pd.read_csv(dataset_path + 'combine_expression.csv')
    labels = pd.read_csv(dataset_path + 'combine_labels.csv')
    domain_labels = pd.read_csv(dataset_path + 'domain_labels.csv')
    data_set = {'datacounts': datacounts.T.values, 'labels': labels.iloc[:, 0].values, 'domain_labels': domain_labels.iloc[:, 0].values}
    return data_set
...
def ScDA(args, data_set):
    ...
    domain_to_indices = np.where(data_set['domain_labels'] == source_name)[0]
    ...
    train_set = {'datacounts': data_set['datacounts'][domain_to_indices], 'labels': data_set['labels'][domain_to_indices],
                 'domain_labels': data_set['domain_labels'][domain_to_indices]}
    domain_to_indices = np.where(data_set['domain_labels'] == target_name)[0]
    print(domain_to_indices)
